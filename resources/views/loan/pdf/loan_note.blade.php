...
<body>
    ...
    <p>En la Ciudad de México, México a {{ $localizedDate }}
    </p>

    <p>Debo y pagaré incondicionalmente a la orden de ANTONIO EMMANUEL FLORES ÁLVAREZ BUYLLA o a quien sus derechos
        represente la cantidad de ${{ $loan_amount }} ({{ strtoupper(amount_in_words($loan_amount)) }} 00/100 M.N.), en
        el domicilio ubicado en Cerrada de Niños Héroes número 26, interior B, Colonia San Pedro Mártir, Alcaldía
        Tlalpan en Ciudad de México, en fecha {{ $localizedEndDate }} ({{ number_in_words($yearToConvert) }}).</p>
